...
if __name__ == "__main__":
    ...
    painted[(0, 0)] = 1
    while True:
        cpu.input([painted[(x, y)]])
        colour = cpu.run()
        if colour == ExitCode.HALT:
            break
        painted[(x, y)] = colour
        turn = cpu.run()
        if turn == ExitCode.HALT:
            break
        dir = (dir + (1 if turn == 1 else -1)) % 4
        if dir == 0:
            y -= 1
        elif dir == 1:
            x += 1
    ...
    minX = min(painted.keys())[0]
    maxX = max(painted.keys())[0]
    minY = min(painted.keys(), key=lambda x: x[1])[1]
    maxY = max(painted.keys(), key=lambda x: x[1])[1]

    print(minX, maxX, minY, maxY)

    painting = [[" " for _ in range(minX, maxX+1)]
                for _ in range(minY, maxY+1)]

    for p, colour in painted.items():
        painting[p[1]-minY][p[0]-minX] = "#" if colour == 1 else " "
